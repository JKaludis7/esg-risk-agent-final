...
const extractJSON = (text) => {
  // Strip any citation tags that leaked through
  let clean = text.replace(/<\/?antml:cite[^>]*>/g, "").replace(/<\/?cite[^>]*>/g, "").replace(/<cite\s+index="[^"]*">/g, "").replace(/<\/cite>/g, "");
  ...
  if (s !== -1 && e !== -1) { const j = clean.substring(s, e + 1); try { return JSON.parse(j); } catch { try { return JSON.parse(j.replace(/,\s*}/g, "}").replace(/,\s*\]/g, "]").replace(/[\x00-\x1F]+/g, " ")); } catch { throw new Error("Could not parse response data"); } } }
  throw new Error("No valid data found in response");
};
const SYS = "You are an expert ESG research analyst. Search thoroughly using multiple specific queries. When a rating or data point is not found in initial results, try searching specifically for it (e.g. search 'Apple MSCI ESG rating' or 'Nike CDP climate score'). For frameworks, assess based on what the company reports even if not explicitly labeled. Always provide substantive detail in every field - never leave descriptions empty. CRITICAL: Respond with ONLY valid JSON. No markdown, no backticks, no extra text, no citation tags, no XML tags. Plain JSON only.";
